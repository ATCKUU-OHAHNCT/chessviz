...
void input(char* p, int* b)
{
    ...
    /*   cout << "\n\n\n"
       ...
       cout << "\n\n\n"; */
    print_desk();
    cout << "\n\nError..\nInput coordinates one more time (X1Y1-X2Y2)\n";
    for (int i = 0; i < 5; i++) {
        cin >> p[i];
        b[i] = p[i];
    }
...
int check_figure_move(int* b)
{
    ...
    if (desk[b[4]][b[3]] == ' ') {
        ...
        if ((desk[b[1]][b[0]] == 'q') && (God_Save_The_Queen(b))
            && ((int)desk[b[4]][b[3]] < 97)) // Белая королева
            return 1;

        if ((desk[b[1]][b[0]] == 'Q') && (God_Save_The_Queen(b))
            && ((int)desk[b[4]][b[3]] > 96)) // Чёрная королева
            return 1;

        if ((desk[b[1]][b[0]] == 'K') && (check_king(b))
            && ((int)desk[b[4]][b[3]] > 96)) // Чёрный король
            return 1;

        if ((desk[b[1]][b[0]] == 'k') && (check_king(b))
            && ((int)desk[b[4]][b[3]] < 97)) // Белый король
            return 1;
    }
